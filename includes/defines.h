...
#define PING_INTERVAL                                                                    \
	60 /* Interval after which to send a ping to client since their last                 \
	      activity */
#define MAX_CONNECTIONS 10

/* Global Modes */
typedef enum e_globalModes {
	OP    = 0x1, /* o: global_op (user is a server operator)
...
typedef enum e_channelModes {
	SECRET       = 0x1,  /* s: secret channel (extends private channel functionality,
	                        cannot be seen by queries for TOPIC, LIST, NAMES) */
	TOPIC_SET_OP = 0x2,  /* t: topic limit (only +qao can set topic) SET DEFAULT */
	INV_ONLY     = 0x4,  /* i: invite only (users may only join channel if they have
	                        been invited by +qao) */
	NO_MSG_IN    = 0x8,  /* n: no external messages (external messages cannot be seen
	                        in channel) SET BY DEFAULT */
...
typedef enum e_memberModes {
	...
	INVIT = 0x2, /* invited (user has been invite to join this channel)
	              * INTERNAL USE ONLY, set through /invite command. */
...
// NOTE: All +qao members automatically inherit (VOICE | BAN_EXEMPT |
// INV_EXEMPT) They cannot be placed on ban list, never need an invite, and can
// always speak
...
b = Bans the nick!ident@host from the channel.
       Extended bantypes are also available. (for more info see extended bans
tutorial) [h] e = Overrides a ban for matching users [h] i = A user must be
invited to join the channel [h] I = Overrides +i for matching users [h] h =
Gives HalfOp status to the user (Limited op access)
...
p = Private channel (for more info see Using Channel Modes to Hide Your Channel)
[o] O = IRC Operator only channel (settable by IRCops) L = Channel link (If +l
is full, the next user will auto-join ) [q] m = Moderated channel (only +vhoaq
users may speak) [h] M = Must be using a registered nick (+r), or have voice
access to talk [o] n = Users outside the channel can not send PRIVMSGs to the
channel [h] K = /KNOCK is not allowed [o] N = No Nickname changes are permitted
in the channel [o] o = Gives Operator status to the user Q = No kicks allowed
[o] s = Secret channel (for more info see Using Channel Modes to Hide Your
Channel) [o] V = /INVITE is not allowed [o]
