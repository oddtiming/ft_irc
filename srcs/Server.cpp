  ...
  : _hostname(hostname), _password(password), _timeStart(std::time(nullptr)), _port(port)
{
	try {
		...
		initializeCommands( );
		std::cout << getTimestamp( ) << GREEN "Command initialization successful" CLEAR
		          << std::endl;
	}
...
void Server::handleConnections( )
{
	...
	/* Set socket option to ensure that we dont attempt to send on a socket that has been
	 * disconnected */
	int yes = 1;
...
void Server::handleMessages(Client* client)
{
	...
	if (client->read( ) <= 0) {
		...
		std::cout << getTimestamp( ) << RED "Removing disconnected client: " CLEAR
		...
		while (rawMessage.empty( ) == false) {
			...
			/* If command is nick but password has not been validated then remove user and break
			 * loop */
...
void Server::executeCommand(const Message& msg)
{
	...
	/* Error message if command is invalid or not supported */
		...
		msg._client->reply(
		  ERR_UNKNOWNCOMMAND(_hostname, msg._client->getNickname( ), msg.getCommand( )));
	}
}
			...
			if (i == 0 && (_pfds[i].revents & POLLIN))
				...
				/* If there is no event, check if ping interval has passed and send PING to */
					...
					if (client->getRegistration( ) && !client->getPingStatus( )
					    && ((std::time(nullptr) - client->getLastActivityTime( ))
					        > PING_INTERVAL)) {
...
void Server::removeClient(Client* client)
{
	...
	for (; it != _channels.end( ); ++it) {
		...
		it->second->removeMember(client,
		                         CMD_PART(client->getNickname( ), it->second->getName( ), ""));
	}
