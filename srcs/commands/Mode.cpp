...
bool Mode::executeMode(char mode, bool removeMode) {
	switch (mode) {
	case 's':
		return _secret(removeMode);
	case 't':
		return _topic(removeMode);
	case 'i':
		return _inviteOrInvis(removeMode);
	case 'n':
		return _no_msg_in(removeMode);
	case 'k':
		return _password(removeMode);
	case 'b':
		return _ban(removeMode);
	case 'o':
...
void Mode::execute(const Message &msg) {
	...
	if (_targetType == CHANNEL && _server->doesChannelExist(_target))
		_server->getChannelPtr(_target)->sendToAll(
...
/* +i - invite only channel mode or invisible global user mode */
bool Mode::_inviteOrInvis(bool removeMode) {
	std::string mode("i");
	...
	if (_targetType == CHANNEL) {
		...
		if (!channelPtr->checkMemberModes(_client, C_OP | OWNER))
			return (_client->reply(
			          ERR_CHANOPRIVSNEEDED(_server->getHostname( ),
			                               _client->getNickname( ),
			                               _target,
	...
	/* If the target is a user, set their mode to +i */
	if (_targetType == USER) {
		Client *clientPtr = _server->getClientPtr(_target);
		if (!clientPtr)
			return false;

		/* Ensure the query comes from the target user */
		if (_client != clientPtr)
			return (_client->reply(ERR_USERSDONTMATCH(_server->getHostname( ), _target)),
			        false);
	...
	if (!channelPtr->checkMemberModes(_client, C_OP | OWNER))
		return (
		  _client->reply(ERR_CHANOPRIVSNEEDED(_server->getHostname( ),
		                                      _client->getNickname( ),
		                                      _target,
		                                      "to set channel mode n (no_msg_in).")),
		  false);
...
bool Mode::_password(bool removeMode) {
	...
	if (!channelPtr->checkMemberModes(_client, C_OP | OWNER))
		return (_client->reply(
		          ERR_CHANOPRIVSNEEDED(_server->getHostname( ),
		                               _client->getNickname( ),
		                               _target,
		                               "to set channel mode k (password protected).")),
		        false);

	if (_params.empty( ))
		return (_client->reply(ERR_NEEDMOREPARAMS(_server->getHostname( ),
		                                          _client->getNickname( ),
		                                          std::string("MODE +k"))),
		        false);
	...
	if (removeMode && pass.compare(channelPtr->getPassword( )))
		return (_client->reply(
		          ERR_KEYSET(_server->getHostname( ), _client->getNickname( ), _target)),
		        false);
...
bool Mode::_ban(bool removeMode) {
	...
	if (!targetMember)
		return (_client->reply(ERR_NOSUCHNICK(
		          _server->getHostname( ), _client->getNickname( ), targetNick)),
		        false);
...
void Mode::sendUserModes(void) {
	...
	if (_client->getNickname( ) != _target) {
		...
		_client->reply(
		  ERR_USERSDONTMATCH(_server->getHostname( ), _client->getNickname( )));
		return;
	}
	/* Reply list of global user modes if users match */
	_client->reply(RPL_UMODEIS(
	  _server->getHostname( ), _client->getNickname( ), _client->getGlobalModes( )));
}
...
void Mode::sendChannelModes(void) {
	...
	if (channel->checkModes(SECRET) && !channel->isMember(_client)) {
		_client->reply(ERR_NOSUCHCHANNEL(
		  _server->getHostname( ), _client->getNickname( ), channel->getName( )));
		return;
	}
	_client->reply(RPL_CHANNELMODEIS(_server->getHostname( ),
	                                 _client->getNickname( ),
	                                 channel->getName( ),
...
void Mode::sendBanList(void) const {
	...
	for (std::vector< std::string >::iterator it = banList.begin( ); it != ite; ++it)
		_client->reply(
		  RPL_BANLIST(_server->getHostname( ), _client->getNickname( ), _target, *it));
	_client->reply(
	  RPL_ENDOFBANLIST(_server->getHostname( ), _client->getNickname( ), _target));
}
...
bool Mode::validateTarget(void) {
	...
	if (_targetType == CHANNEL && !_server->doesChannelExist(_target)) {
		_client->reply(
		  ERR_NOSUCHCHANNEL(_server->getHostname( ), _client->getNickname( ), _target));
		return false;
	}
...
/*
        ...
        Reply will contain all modes successfully changed in the order they were
   received (if mode was already set it will be removed from reply) Additionally
   another error for each mode that fails will be sent

        Irssi will not send /mode with no parameters - test with other clients.
   IF this is consistent then do nothing and ignore commands
*/